...
test('production V2 authority cutover routes the default learning surfaces', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Profile & capability' }).click()
  await expect(page.getByRole('heading', { name: 'Profile & Capability' })).toBeVisible()
  await expect(page.getByRole('link', { name: 'Legacy Today history' })).toBeVisible()
  await page.getByRole('link', { name: 'Legacy Roadmap history' }).click()
