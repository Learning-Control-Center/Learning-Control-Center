...
export function TodayV2Page() {
  ...
  return <div className="mx-auto w-full max-w-[86rem] space-y-6">
    ...
    <PageHeader eyebrow="Daily control loop" title="Today" description="Choose a sensible next action, continue actual work, or do something else. Skipping, replacement, and expiry create no debt." actions={<div className="flex flex-wrap items-end gap-2"><label className="text-xs font-medium text-ink/65">Available minutes (optional)<input className="mt-1 block w-44 rounded-xl border border-ink/15 bg-white px-3 py-2 text-sm" min="0" step="5" type="number" value={availableMinutes} placeholder="Unknown" onChange={(event) => setAvailableMinutes(event.target.value)} /></label><Button disabled={Boolean(working)} aria-busy={working === 'generation'} onClick={() => void generationCommand()}><RefreshCw className={`size-4 ${working === 'generation' ? 'animate-spin' : ''}`} />{today.generation ? 'Regenerate explicitly' : 'Generate Today'}</Button></div>} />
    ...
    {today.generation ? <><Surface className="grid gap-4 p-5 sm:grid-cols-3"><div><p className="eyebrow">Local day</p><p className="mt-1 text-sm">{today.generation.localDate} · {today.generation.timezone}</p></div><div><p className="eyebrow">Generation</p><p className="mt-1 text-sm">#{today.generation.generationSequence}{today.generation.regeneration ? ' · explicit regeneration' : ''}</p></div><div><p className="eyebrow">Advisory portfolio</p><p className="mt-1 text-sm">{today.generation.suggestions.length} item{today.generation.suggestions.length === 1 ? '' : 's'}</p></div></Surface><section aria-labelledby="today-portfolio-heading"><SectionHeader headingId="today-portfolio-heading" title="What makes sense now" description="Primary, complementary, and maintenance roles come from Recommendation V2 policy. They are advice, not assignments." /><div className="mt-4 space-y-4">{today.generation.suggestions.map((suggestion) => <SuggestionCard key={suggestion.id} suggestion={suggestion} sessions={sessions} sourceLink={sourceDestination(suggestion, sourceLinks)} activeSessionId={active?.id ?? null} working={working} command={command} correct={correctLatestInteraction} />)}{!today.generation.suggestions.length ? <EmptyState title="No useful eligible work" detail="The empty advisory portfolio is preserved truthfully; no recommendation was manufactured." /> : null}</div></section></> : null}
