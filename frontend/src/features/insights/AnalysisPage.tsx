...
export function AnalysisPage() {
  ...
  const renderSignal = (signal: AnalysisSnapshot['signals'][number]) => {
  ...
  return <div className="mx-auto w-full max-w-[90rem] space-y-6">
    ...
    <Surface className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-3"><Fact label="Snapshot" value={isHistorical ? 'Historical snapshot' : label(current.status)} /><Fact label="Completeness" value={label(selected.completeness)} /><Fact label="Signals" value={String(selected.signals.length)} /><Fact label="Competency gaps" value={String(selected.gaps.length)} /><Fact label="Exclusive cutoff" value={new Date(selected.cutoffAt).toLocaleString()} /><Fact label="Completed through" value={selected.completedThroughDate} /></Surface>
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="space-y-5">
        <Surface className="p-5"><SectionHeader title="Diagnostic signals" description="The most immediate signals lead. Expand the remainder only when deeper diagnosis is useful." />{selected.signals.length ? <><div className="mt-4 grid gap-3 md:grid-cols-2">{selected.signals.slice(0, summaryLimit).map(renderSignal)}</div>{selected.signals.length > summaryLimit ? <details className="mt-4 rounded-xl border border-ink/10 bg-white/50 p-3"><summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-moss">Show {selected.signals.length - summaryLimit} more diagnostic signals</summary><div className="mt-3 grid gap-3 md:grid-cols-2">{selected.signals.slice(summaryLimit).map(renderSignal)}</div></details> : null}</> : <p className="mt-4 text-sm text-ink/65">No diagnostic signals in this snapshot.</p>}</Surface>
        <Surface className="p-5"><SectionHeader title="Competency gaps" description="The first target comparisons show the current shape of the gap. Expand for the complete diagnostic set." />{selected.gaps.length ? <><div className="mt-4 space-y-3">{selected.gaps.slice(0, summaryLimit).map(renderGap)}</div>{selected.gaps.length > summaryLimit ? <details className="mt-4 rounded-xl border border-ink/10 bg-white/50 p-3"><summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-moss">Show {selected.gaps.length - summaryLimit} more competency gaps</summary><div className="mt-3 space-y-3">{selected.gaps.slice(summaryLimit).map(renderGap)}</div></details> : null}</> : <p className="mt-4 text-sm text-ink/65">No target gaps are present.</p>}</Surface>
        {selected.unknownMarkers.length ? <Surface className="p-5"><SectionHeader title="Unknown inputs" description="Unknown remains an explicit state, not an error or zero. The first items establish the pattern; expand for the complete set." /><ul className="mt-4 space-y-2">{selected.unknownMarkers.slice(0, summaryLimit).map(renderUnknown)}</ul>{selected.unknownMarkers.length > summaryLimit ? <details className="mt-4 rounded-xl border border-ink/10 bg-white/50 p-3"><summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-moss">Show {selected.unknownMarkers.length - summaryLimit} more Unknown inputs</summary><ul className="mt-3 space-y-2">{selected.unknownMarkers.slice(summaryLimit).map(renderUnknown)}</ul></details> : null}</Surface> : null}
        <Surface className="p-5"><SectionHeader title="Normalized diagnostic facts" description="Frozen inputs are retained for audit, not promoted as new explanation." /><div className="mt-4"><AuditDisclosure label={`Open normalized fact audit · ${selected.normalizedFacts.length} fact${selected.normalizedFacts.length === 1 ? '' : 's'}`}><div className="space-y-2">{selected.normalizedFacts.map((fact) => <AuditDisclosure key={fact.stable_key} label={`${label(fact.fact_type)} · ${subjectTitle(fact.subject_type, fact.subject_id)}`}><p>Subject ID: {fact.subject_id}</p><pre className="mt-2 overflow-auto">{JSON.stringify(fact.payload, null, 2)}</pre></AuditDisclosure>)}</div></AuditDisclosure></div></Surface>
      </div>
      <aside className="space-y-5"><Surface className="p-5"><div className="mb-4 flex items-center gap-2"><History className="size-4 text-moss" /><h2 className="font-display text-lg font-semibold">Snapshot history</h2></div><ul className="max-h-[32rem] space-y-2 overflow-y-auto pr-1">{history.map((item) => <li key={item.id}><button aria-pressed={selected.id === item.id} className={`w-full rounded-xl border p-3 text-left ${selected.id === item.id ? 'border-moss/40 bg-moss/10' : 'border-ink/10 bg-white/60'}`} disabled={Boolean(inspecting)} onClick={() => void inspect(item.id)}><span className="block text-sm font-medium capitalize">{label(item.purpose)}</span><span className="mt-1 flex items-center gap-1 text-xs text-ink/65"><Clock3 className="size-3" />{new Date(item.generatedAt).toLocaleString()}</span><span className="mt-1 block text-xs capitalize text-ink/65">{label(item.completeness)}{selected.id === item.id ? ' · Selected' : ''}</span></button></li>)}</ul></Surface><AuditDisclosure label="Policy and snapshot lineage"><p>{selected.lineage?.analysis_algorithm_version ?? 'Algorithm version Unknown'}</p><p>{selected.lineage?.analysis_policy_version ?? 'Analysis policy version Unknown'}</p><p>{selected.lineage?.normalization_schema_version ?? 'Normalization schema version Unknown'}</p><p className="mt-2 font-mono">Output {selected.outputHash}</p><p className="font-mono">Input {selected.inputHash}</p></AuditDisclosure></aside>
    </div>
  </div>
}
