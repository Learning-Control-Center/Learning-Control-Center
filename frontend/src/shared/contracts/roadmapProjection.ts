export type RoadmapProfileTarget = {
  ...
  comparisonStateReference?: { snapshotId: string; gapStableKey: string; comparisonStatus: string } | null
}
...
export type RoadmapProjectionNode = {
  ...
  capability: { scopes: { scopeKey: string; assessmentStatus: string; confidence: string; freshness: string; reviewDue: boolean | null; levelId?: string | null; levelKey?: string | null; levelTitle?: string | null }[] }
...
export type RoadmapProjectionEdge = {
  ...
  satisfaction: { aggregate_state: string; unknown_reasons: string[]; criterion_states?: { criterionDefinitionId: string; state: string }[] }
}
...
export function adaptRoadmapProjectionNode(node: RoadmapProjectionNode): CompatibleRoadmapProjectionNode {
  const targets = node.profileTargets ?? (node.profileTarget ? [node.profileTarget] : [])
  return { ...node, profileTargets: targets, isTargeted: node.isTargeted ?? node.isCurrent, layoutLane: node.layoutLane ?? (node.profileDomain ? { id: node.profileDomain.id, title: node.profileDomain.title, orderIndex: node.profileDomain.orderIndex } : { id: 'graph-foundations', title: 'Graph foundations', orderIndex: 999 }) }
}
