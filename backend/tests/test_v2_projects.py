...
async def test_project_lifecycle_blockers_evidence_and_capability_separation(
    ...
    assert candidates["build"]["intended_independence_modes"] == [
        "assisted",
        "guided",
        "independent",
    ]
    assert len(candidates["build"]["evidence_opportunities"]) == 1
    assert candidates["build"]["evidence_opportunities"][0]["task_definition_id"] is not None
    assert (
        candidates["build"]["evidence_opportunities"][0]["project_criterion_definition_id"]
        is not None
    )
    assert candidates["release"]["evidence_opportunities"] == []
    ...
    with pytest.raises(AppError, match="derived characteristics"):
        _validate_portable_payload(
            tampered_actuality, "tampered-project-actuality", schema_version=10
        )
    ...
    with pytest.raises(AppError, match="derived characteristics"):
        _validate_portable_payload(
            malformed_characteristics, "malformed-project-characteristics", schema_version=10
        )
...
async def test_project_cutoff_corrections_idempotency_and_portable_v4(
    ...
    exact_reference = next(
        item for item in exact.active_version_references if item.project_id == project["id"]
    )
    after_reference = next(
        item for item in after.active_version_references if item.project_id == project["id"]
    )
    assert exact_reference.project_event_sequence_cutoff == event_row.event_sequence - 1
    assert exact_reference.latest_project_event_id != event_row.id
    assert after_reference.project_event_sequence_cutoff == event_row.event_sequence
    assert after_reference.latest_project_event_id == event_row.id
    assert (
    ...
    _validate_portable_payload(
        selective_package["payload"], "cross-project-closure", schema_version=10
    )
    ...
    assert package["schemaVersion"] == 10
    ...
    with pytest.raises(AppError, match="unsupported evaluation policy"):
        _validate_portable_payload(
            unsupported_policy, "unsupported-project-criterion-policy", schema_version=10
        )

    reversed_self_references = deepcopy(package["payload"])
    reversed_self_references["tables"]["project_events"].reverse()
    _validate_portable_payload(
        reversed_self_references, "reversed-project-self-references", schema_version=10
    )
    ...
    with pytest.raises(AppError, match="definition envelope"):
        _validate_portable_payload(
            tampered_definition, "tampered-project-definition", schema_version=10
        )
    ...
    with pytest.raises(AppError, match="rebuilt Project catalog"):
        _apply_portable_restore(
            db, tampered, True, package_id="tampered-project", schema_version=10
        )
    db.rollback()
    db.expire_all()
...
def test_v3_to_v4_adapter_is_empty_and_project_dependencies_are_public(db: Session) -> None:
    ...
    for table_name in PORTABLE_V5_GRAPH_PROJECTION_TABLES:
        payload["tables"].pop(table_name)
