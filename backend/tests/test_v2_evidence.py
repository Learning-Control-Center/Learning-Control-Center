...
async def test_verification_application_service_creates_result_and_context_evidence(
    ...
    _validate_portable_payload(_portable_payload(db), "deleted-source-retained", schema_version=8)
...
async def test_portable_evidence_history_round_trips_and_rejects_lineage_tampering(
    ...
    portable = _portable_payload(db)
    tables, _summary = _validate_portable_payload(portable, "evidence-round-trip", schema_version=8)
    ...
    with pytest.raises(AppError, match="Evidence backfill lineage"):
        _validate_portable_payload(tampered, "evidence-tampered", schema_version=8)
    ...
    with pytest.raises(AppError, match="Evidence provenance is inconsistent"):
        _validate_portable_payload(invalid_policy, "evidence-policy-tampered", schema_version=8)
    ...
    with pytest.raises(AppError, match="Evidence provenance is inconsistent"):
        _validate_portable_payload(invalid_source, "evidence-source-tampered", schema_version=8)
    ...
    with pytest.raises(AppError, match="Evidence provenance is inconsistent"):
        _validate_portable_payload(invalid_origin, "evidence-origin-tampered", schema_version=8)
    ...
    with pytest.raises(AppError, match="credential-bearing external reference"):
        _validate_portable_payload(unsafe_reference, "unsafe-evidence-reference", schema_version=8)
    ...
    with pytest.raises(AppError, match="EvidenceLink is inconsistent"):
        _validate_portable_payload(
            invalid_link_provenance, "evidence-link-provenance-tampered", schema_version=8
        )
