...
Current application-produced portable backups use `schemaVersion: 7`. V3 introduced Curriculum, V4 Projects, V5 the native Learning Graph and Roadmap Projection inputs, and V6 immutable discipline-configuration and Analysis V3 history. V7 adds complete immutable Recommendation V2 runs, frozen input envelopes, candidates, eligibility/rule results, ExpectedLearningValue, all score components, selection decisions, selected recommendations, and explanation reasons. The Recommendation history checkpoint verifies exact input, policy-bundle, output, and child-history hashes without generating replacement recommendations. V1 through V6 backups remain readable through lossless linear adapters; the V6-to-V7 adapter initializes Recommendation V2 history empty and never reconstructs V1 recommendation history. Native graph edges, diagnostics, capability, or recommendation decisions are never fabricated from legacy status or Roadmap structure. Derived capability/review, availability, graph satisfaction, Roadmap Projection cache, and Analysis current-pointer state remain rebuildable rather than portable truth.
...
```json
{
  ...
  "packageId": "example-empty-portable-v7",
  "appVersion": "1.0.0",
  "createdAt": "2026-09-04T18:30:00.000Z",
  "payload": {
    "manifest": {
      ...
      "restoreActions": ["clear_projection_invalidations", "rebuild_capability_states", "rebuild_review_states", "verify_projection_hash_parity", "rebuild_curriculum_availability", "verify_curriculum_catalog_hash_parity", "rebuild_project_current_state", "rebuild_project_task_availability", "verify_project_catalog_hash_parity", "rebuild_learning_graph_satisfaction", "rebuild_roadmap_projection", "verify_roadmap_projection_hash_parity", "verify_legacy_roadmap_active_state_parity", "rebuild_analysis_v3_current_pointer", "verify_analysis_v3_history_hash_parity", "verify_recommendation_v2_history_hash_parity"]
    },
...
| --- | --- | --- | --- | --- |
| `schemaVersion` | integer | yes | `1` for V1 packages; portable backup/restore accepts `1` through `7` and exports `7` | `7` |
