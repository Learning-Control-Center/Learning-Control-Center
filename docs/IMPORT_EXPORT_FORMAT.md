...
Current application-produced portable backups use `schemaVersion: 6`. V3 introduced the canonical Curriculum domain. V4 adds stable Project identities, immutable Project versions and definitions, activation and lifecycle/blocker history, Activity and Session Project attribution, Project Evidence lineage, and Evidence-backed ProjectCriterion evaluation history. V5 adds native Learning Graph identity/version/edge facts and activation history, exact legacy Roadmap active-state compatibility, and non-semantic Roadmap Projection position/preferences input. V6 adds immutable discipline-configuration history and immutable Analysis V3 run, snapshot-detail, normalized-fact, gap, signal, unknown-marker, and policy-lineage facts. Native V2 graph edges are never fabricated from V1 prerequisite history; the V1-to-V2 view remains a labeled compatibility adapter with `legacy_verified` semantics. Project Evidence retains `project-evidence-policy/v1` derivation facts: an artifact reference alone is never Strong or High-confidence Evidence; Strong requires a completed actual Activity, an artifact, and an explicit passing result against the opportunity's ProjectCriterion, while attribution provenance yields at most Medium source confidence. Restore recomputes those facts from canonical Activity/Session history at the Evidence-recording cutoff. Project lifecycle, task availability, graph satisfaction, the Roadmap Projection cache, and the Analysis V3 current pointer are rebuildable and are never exported as capability truth. The payload carries capability, Curriculum, Project, Roadmap Projection, and Analysis V3 current-pointer checkpoints; they bind rebuildable state to immutable lineage, exclusive cutoffs, explicit policy versions, source/output hashes, active-version references, and per-item hashes. The manifest explicitly omits current capability/review projections, Curriculum availability, Project current/availability views, graph satisfaction, Roadmap Projection cache/checkpoint rows, Analysis V3 current-pointer rows, and `projection_invalidations`; restore clears the queue, validates retained lineage, deterministically rebuilds projections and the Analysis pointer, and requires checkpoint parity before commit. Frozen V1 through V5 portable backups remain accepted through linear schema-version dispatch. V1 conversion adds built-in Technical/CEFR scale facts, unknown-preserving legacy assertions, deterministic Activities/SessionContribution, and provenance-bearing Evidence without inferred strength, source confidence, criterion scope, Curriculum, Project, graph, Analysis V3 history, or capability. The V2-to-V3 adapter initializes Curriculum tables empty; the V3-to-V4 adapter initializes Project tables empty; the V4-to-V5 adapter initializes native graph and manual projection-input tables empty and deterministically mirrors preserved V1 Roadmap pointers into compatibility state. The V5-to-V6 adapter creates an exact immutable baseline from the retained discipline profile when one exists and otherwise initializes Analysis V3 history empty; it never fabricates diagnostics or executes Analysis. No adapter fabricates semantic mappings from legacy titles, notes, activity categories, Tracks, exit criteria, or Roadmap structure. Older valid V1 backups that also lack `roadmap_scope_events` are identified with `portableCompatibility: "legacy_scope_baseline"`; restore creates at most one deterministic current-scope baseline and never invents earlier phase changes.
...
```json
{
  ...
  "packageId": "example-empty-portable-v6",
  "appVersion": "1.0.0",
  "createdAt": "2026-09-04T18:30:00.000Z",
  "payload": {
    "manifest": {
      ...
      "omittedRebuildableState": ["competency_capability_states", "competency_review_states", "projection_invalidations", "curriculum_availability", "project_current_lifecycle", "project_task_availability", "learning_graph_edge_satisfaction", "roadmap_projection_cache", "roadmap_projection_checkpoint", "analysis_v3_current_state"],
      "restoreActions": ["clear_projection_invalidations", "rebuild_capability_states", "rebuild_review_states", "verify_projection_hash_parity", "rebuild_curriculum_availability", "verify_curriculum_catalog_hash_parity", "rebuild_project_current_state", "rebuild_project_task_availability", "verify_project_catalog_hash_parity", "rebuild_learning_graph_satisfaction", "rebuild_roadmap_projection", "verify_roadmap_projection_hash_parity", "verify_legacy_roadmap_active_state_parity", "rebuild_analysis_v3_current_pointer", "verify_analysis_v3_history_hash_parity"]
    },
...
| --- | --- | --- | --- | --- |
| `schemaVersion` | integer | yes | `1` for V1 packages; portable backup/restore accepts `1` through `6` and exports `6` | `6` |
